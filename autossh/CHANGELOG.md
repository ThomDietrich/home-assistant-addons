...
- Fix autossh silently retrying failed connections instead of exiting. Set `AUTOSSH_GATETIME=0` and `AUTOSSH_MAXSTART=1` to ensure clean exit on connection failure. Fixes https://github.com/ThomDietrich/home-assistant-addons/issues/36
- Convert to s6-overlay supervised service for automatic restart on any failure, including pre-flight check failures
- Make the `-N` parameter permanent and remove it from the configurable `other_ssh_options`. Existing users with `-N` in their `other_ssh_options` should remove it, as it is now always applied. Resolves https://github.com/ThomDietrich/home-assistant-addons/issues/35
- Provide a default empty list for `remote_forwarding`. This should resolve https://github.com/ThomDietrich/home-assistant-addons/issues/33
